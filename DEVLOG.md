...
## 2026-08-15 — Executive Pulse decision workspace

- Rebuilt Executive Pulse as a compact decision workspace that answers operating state, material change, evidence limitations, and required evidence action before presenting supporting detail. The surface reuses the Reach command language while keeping a distinct decision purpose.
- Added a pure deterministic web-layer projection over `OverviewReadModelV1`. It preserves exact values and authored briefing fields, computes traffic coverage and collection lag from contract evidence, applies explicit failed/stale/partial/empty/complete precedence, and never infers causation, attribution, conversion, unique visitors, or unsupported comparisons.
- Replaced the animated hero and duplicated warnings with four exact fact cards, Changed/Known/Limited/Action rows, one deduplicated attention list, Evidence health, and Evidence actions. Critical source exceptions remain visible, unhealthy collection states cannot claim that no action is required, and partial traffic coverage is represented as a limitation.
- Generalized the command header and shell workspace treatment without changing the Delivery surface or the deployed Reach height contract. Executive Pulse retains one page heading, exposes the exact observation window, uses static first-frame values, and keeps evidence links and refresh operations on the existing safe server boundary.
- Verified 1586 px, 1440 px, 768 px, 390 px, and 320 px layouts with no document-level horizontal overflow. The desktop decision layer fits the initial viewport, mobile controls retain 44 px targets, reduced motion does not hide content, and unit, axe, type, lint, formatting, production-build, and Playwright gates pass.

## 2026-08-15 — Executive evidence motion and timezone settings

- Added four restrained Executive Pulse microcharts using only evidence already present in `OverviewReadModelV1`: supported Stars and Open issues comparisons, directly observed traffic-day coverage, and collection lag against the supplied stale threshold. Exact values remain static and first-frame visible; missing comparisons render an honest endpoint or unavailable state rather than a fabricated sparkline.
